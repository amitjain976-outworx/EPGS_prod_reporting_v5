...
--  Run against: epgs_reporting_gold (TARGET / local DB)
--  Safe to run on fresh DB — uses CREATE TABLE IF NOT EXISTS
--
--  CHANGES vs previous version:
--    ✅ ADDED   : dim_policy  (new dimension — sourced from business_policy)
--    ✅ ADDED   : fact_parking_session.policy_key              BIGINT NULL
--    ✅ ADDED   : fact_parking_session.validation_refund_flag  TINYINT(1) NOT NULL DEFAULT 0
--    ✅ ADDED   : fact_reservation.policy_key                  BIGINT NULL
--    ✅ ADDED   : fact_permit_subscription.policy_key          BIGINT NULL
--    ✅ ADDED   : fact_passes.policy_key                       BIGINT NULL
--    ✅ ADDED   : fact_payment.tax_exempt_flag                 ENUM('0'..'9') NULL
--    ✅ ADDED   : fact_payment.sales_tax_exemption             DECIMAL(12,2) NULL
--    ✅ ADDED   : fact_payment.sales_tax_collected             DECIMAL(12,2) NULL
--    ✅ ADDED   : fact_payment.validate_refund_amount          DECIMAL(10,2) NULL
--    ✅ ADDED   : fact_payment.vr_anet_trans_id                INT NULL
--    ✅ ADDED   : fact_payment.vr_refund_status                ENUM('PENDING','FAILED','REFUNDED') NULL
-- =============================================================================
...
-- -----------------------------------------------------------------------------
--  fact_reservation
--  ✅ ADDED: policy_key BIGINT NULL  (FK to dim_policy; NULL in initial load)
-- -----------------------------------------------------------------------------
...
--  ✅ ADDED: policy_key            BIGINT NULL
--            Mapped from tickets.policy_id → dim_policy.policy_key (SOURCE 1 only).
--            NULL for overstay_tickets and ticket_extends sources.
--  ✅ ADDED: validation_refund_flag TINYINT(1) NOT NULL DEFAULT 0
--            1 = ticket has at least one row in validation_refunds; 0 = none.
--            Checked for all three session sources (tickets / overstay / extends).
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS fact_parking_session (
    canonical_session_key   BIGINT        AUTO_INCREMENT PRIMARY KEY,
    source_id               BIGINT        NOT NULL
        COMMENT 'Raw PK from tickets / overstay_tickets / ticket_extends — no offset',
    ...
    policy_key              BIGINT        NULL
        COMMENT 'FK to dim_policy; mapped from tickets.policy_id (NULL for overstay/extend)',
...
-- -----------------------------------------------------------------------------
--  fact_payment
--  ✅ ADDED: tax_exempt_flag        ENUM('0'..'9') NULL
--            Mapped from tickets.paid_type; NULL for all non-ticket sources.
--  ✅ ADDED: sales_tax_exemption    DECIMAL(12,2) NULL DEFAULT 0
--            Tax exemption amount; hardcoded 0 in this load.
--  ✅ ADDED: sales_tax_collected    DECIMAL(12,2) NULL DEFAULT 0
--            Tax collected amount; hardcoded 0 in this load.
--  ✅ ADDED: validate_refund_amount DECIMAL(10,2) NULL
--            validation_refunds.total matched by reference_key = ticket_number.
--            NULL for all non-ticket sources and tickets with no refund record.
--  ✅ ADDED: vr_anet_trans_id       INT NULL
--            validation_refunds.anet_transaction_id (same join as above).
--            NULL for all non-ticket sources and tickets with no refund record.
--  ✅ ADDED: vr_refund_status       ENUM('PENDING','FAILED','REFUNDED') NULL
--            validation_refunds.transaction_status (same join as above).
--            NULL for all non-ticket sources and tickets with no refund record.
--  ✅ NOTE : amount is NULL when no anet_transaction row exists for the source row.
-- -----------------------------------------------------------------------------
